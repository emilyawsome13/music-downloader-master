...
def test_song_from_url_without_genres(monkeypatch):
    """
    Tests if Song.from_url() tolerates missing genres in Spotify metadata.
    """
    ...
    monkeypatch.setattr(
        ...
        lambda self, _: {
            "duration_ms": 188000,
            "name": "Ropes",
            "artists": [{"id": "artist-id", "name": "Dirty Palm"}],
            "album": {"id": "album-id"},
            "disc_number": 1,
            "track_number": 1,
            "id": "track-id",
            ...
            "external_ids": {"isrc": "GB2LD2110301"},
            "popularity": 0,
        },
    )
    monkeypatch.setattr(
