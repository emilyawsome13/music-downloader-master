...
def test_embed_metadata_without_isrc(tmpdir, monkeypatch):
    """
    Test embedding metadata when a song has no ISRC.
    """
    ...
    song = Song.from_dict(
        {
            ...
            "cover_url": "https://i.scdn.co/image/ab67616d0000b273fe2cb38e4d2412dbb0e54332",
            "explicit": False,
            "download_url": "https://music.youtube.com/watch?v=test-video",
            "artist": "Dirty Palm",
            "copyright_text": "",
            "publisher": "",
            "url": "https://music.youtube.com/watch?v=test-video",
            "popularity": 0,
        }
    )
    ...
    file_metadata = get_file_metadata(output_file)
    assert file_metadata is not None
    assert file_metadata["name"] == "Ropes"
    assert file_metadata["url"] == "https://music.youtube.com/watch?v=test-video"
